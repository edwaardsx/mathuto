...
import com.capstone.mathuto.Main.Companion.WATCHED_TUTORIAL12
import com.capstone.mathuto.R
import com.capstone.mathuto.databinding.ActivityLessonTwelveBinding
...
import com.capstone.mathuto.timer.QuizTimer12
import com.capstone.mathuto.watch.ModuleEleven
import com.capstone.mathuto.watch.ModuleTwelve
import java.util.*

class Lesson12 : AppCompatActivity(), TextToSpeech.OnInitListener {

    private lateinit var binding: ActivityLessonTwelveBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLessonTwelveBinding.inflate(layoutInflater)
        ...
        binding.btnWatchTutorial.setOnClickListener {
            val intent = Intent(applicationContext, ModuleTwelve::class.java)
            WATCHED_TUTORIAL12 = true
        ...
        binding.btnStartQuiz.setOnClickListener {
            val intent = Intent(applicationContext, com.capstone.mathuto.timer.QuizTimer12::class.java)
            intent.flags = Intent.FLAG_ACTIVITY_NEW_TASK
            applicationContext.startActivity(intent)
            overridePendingTransition(0, 0)
        }
        ...
        binding.btnTts2.setOnClickListener {
            ...
            val lessonDescriptions = "$text2 $text3 $text4 $text5 $text6 $text7 $text8 $text9"
            if (lessonDescriptions.isNotEmpty()) {
                if (isReadingAloud) {
                    tts.stop()
                    isReadingAloud = false
        ...
        binding.btnTts3.setOnClickListener {
            val text10 = binding.tvLessonsDescriptionTen.text.toString()
            val lessonDescriptions = "$text10"
            if (lessonDescriptions.isNotEmpty()) {
                if (isReadingAloud) {
                    tts.stop()
                    isReadingAloud = false
    ...
    private fun showObjectivesDialog() {
        ...
        dialog.setContentView(R.layout.activity_objectives_twelve)
